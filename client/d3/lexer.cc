...
const char *idLexer::GetFileName(void) {
...
void idLexer::SetFlags(int flags) {
  idLexer::flags = flags;
}

int idLexer::GetFlags(void) {
...
void idLexer::Error(const char *str, ...) {
  ...
  if (idLexer::flags & LEXFL_NOFATALERRORS) {
    warning_ln("file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text);
  } else {
    die("file %s, line %d: %s", idLexer::filename.c_str(), idLexer::line, text);
  }
}
...
int idLexer::ReadToken(idToken *token) {
  ...
  if (!loaded) {
    die("idLexer::ReadToken: no file loaded");
    return 0;
  }
...
void idLexer::UnreadToken(const idToken *token) {
  if (idLexer::tokenavailable) {
    die("idLexer::unreadToken, unread token twice\n");
  }
  idLexer::token = *token;
...
int idLexer::LoadFile(const char *filename, bool OSPath) {
  ...
  if (idLexer::loaded) {
    die("idLexer::LoadFile: another script already loaded");
    return false;
  }
  ...
  buf[length] = '\0';
  memcpy(buf, fp.buffer, fp.length);
  idLexer::fileTime = 0;
...
int idLexer::LoadMemory(const char *ptr, int length, const char *name, int startLine) {
  if (idLexer::loaded) {
    die("idLexer::LoadMemory: another script already loaded");
    return false;
  }
