...
idMatX::idMatX( int rows, int columns ) {
  numRows = numColumns = alloced = 0;
  mat = NULL;
  SetSize( rows, columns );
}

idMatX::idMatX( int rows, int columns, float *src ) {
  numRows = numColumns = alloced = 0;
  mat = NULL;
  SetData( rows, columns, src );
}

void idMatX::Set( int rows, int columns, const float *src ) {
  SetSize( rows, columns );
  memcpy( this->mat, src, rows * columns * sizeof( float ) );
}

void idMatX::Set( const idMat3 &m1, const idMat3 &m2 ) {
  int i, j;
...
void idMatX::Set( const idMat3 &m1, const idMat3 &m2, const idMat3 &m3, const idMat3 &m4 ) {
  int i, j;
...
const float *idMatX::operator[]( int index ) const {
  assert( ( index >= 0 ) && ( index < numRows ) );
  return mat + index * numColumns;
}

float *idMatX::operator[]( int index ) {
  assert( ( index >= 0 ) && ( index < numRows ) );
  return mat + index * numColumns;
}

idMatX &idMatX::operator=( const idMatX &a ) {
...
idMatX idMatX::operator*( const float a ) const {
  idMatX m;
...
idVecX idMatX::operator*( const idVecX &vec ) const {
  idVecX dst;
...
idMatX idMatX::operator*( const idMatX &a ) const {
  idMatX dst;
...
idMatX &idMatX::operator*=( const float a ) {
#ifdef MATX_SIMD
...
idMatX operator*( const float a, idMatX const &m ) {
  return m * a;
}
...
bool idMatX::Compare( const idMatX &a ) const {
  int i, s;
...
bool idMatX::Compare( const idMatX &a, const float epsilon ) const {
  int i, s;
  ...
  s = numRows * numColumns;
  for ( i = 0; i < s; i++ ) {
    if ( qkmath::fabs( mat[i] - a.mat[i] ) > epsilon ) {
      return false;
    }
  }
  return true;
...
bool idMatX::operator==( const idMatX &a ) const {
  return Compare( a );
}
...
void idMatX::SetSize( int rows, int columns ) {
...
void idMatX::SetTempSize( int rows, int columns ) {
  int newSize;
...
void idMatX::SetData( int rows, int columns, float *data ) {
  assert( mat < idMatX::tempPtr || mat > idMatX::tempPtr + MATX_MAX_TEMP );
...
void idMatX::Zero( int rows, int columns ) {
  SetSize( rows, columns );
#ifdef MATX_SIMD
  SIMDProcessor->Zero16( mat, numRows * numColumns );
#else
...
void idMatX::Identity( int rows, int columns ) {
  assert( rows == columns );
  SetSize( rows, columns );
  idMatX::Identity();
}

void idMatX::Diag( const idVecX &v ) {
...
void idMatX::Random( int seed, float l, float u ) {
  int i, s;
  float c;
  idRandom rnd(seed);
...
void idMatX::Random( int rows, int columns, int seed, float l, float u ) {
  int i, s;
  float c;
  idRandom rnd(seed);
...
void idMatX::Clamp( float min, float max ) {
  int i, s;
  s = numRows * numColumns;
  for ( i = 0; i < s; i++ ) {
    if ( mat[i] < min ) {
...
idMatX &idMatX::SwapRows( int r1, int r2 ) {
  float *ptr;
...
idMatX &idMatX::SwapColumns( int r1, int r2 ) {
  int i;
  float tmp, *ptr;
...
idMatX &idMatX::SwapRowsColumns( int r1, int r2 ) {
...
void idMatX::ClearUpperTriangle( void ) {
  assert( numRows == numColumns );
...
void idMatX::ClearLowerTriangle( void ) {
  assert( numRows == numColumns );
...
void idMatX::SquareSubMatrix( const idMatX &m, int size ) {
  int i;
...
float idMatX::MaxDifference( const idMatX &m ) const {
  int i, j;
  float diff, maxDiff;
  ...
  for ( i = 0; i < numRows; i++ ) {
    for ( j = 0; j < numColumns; j++ ) {
      diff = qkmath::fabs( mat[ i * numColumns + j ] - m[i][j] );
...
bool idMatX::IsZero( const float epsilon ) const {
  // returns true if (*this) == Zero
  for ( int i = 0; i < numRows; i++ ) {
    for ( int j = 0; j < numColumns; j++ ) {
      if ( qkmath::fabs( mat[i * numColumns + j] ) > epsilon ) {
        return false;
      }
    }
  }
...
bool idMatX::IsIdentity( const float epsilon ) const {
  // returns true if (*this) == Identity
  assert( numRows == numColumns );
  for ( int i = 0; i < numRows; i++ ) {
    for ( int j = 0; j < numColumns; j++ ) {
      if ( qkmath::fabs( mat[i * numColumns + j] - (float)( i == j ) ) > epsilon ) {
        return false;
      }
    }
  }
...
bool idMatX::IsDiagonal( const float epsilon ) const {
  // returns true if all elements are zero except for the elements on the diagonal
  assert( numRows == numColumns );
  for ( int i = 0; i < numRows; i++ ) {
    for ( int j = 0; j < numColumns; j++ ) {
      if ( i != j && qkmath::fabs( mat[i * numColumns + j] ) > epsilon ) {
        return false;
      }
    }
  }
...
bool idMatX::IsTriDiagonal( const float epsilon ) const {
  ...
  for ( int i = 0; i < numRows-2; i++ ) {
    for ( int j = i+2; j < numColumns; j++ ) {
      if ( qkmath::fabs( (*this)[i][j] ) > epsilon ) {
        return false;
      }
      if ( qkmath::fabs( (*this)[j][i] ) > epsilon ) {
        return false;
      }
    }
  }
...
bool idMatX::IsSymmetric( const float epsilon ) const {
  // (*this)[i][j] == (*this)[j][i]
  if ( numRows != numColumns ) {
    return false;
  }
  for ( int i = 0; i < numRows; i++ ) {
    for ( int j = 0; j < numColumns; j++ ) {
      if ( qkmath::fabs( mat[ i * numColumns + j ] - mat[ j * numColumns + i ] ) > epsilon ) {
        return false;
      }
    }
  }
...
float idMatX::Determinant( void ) const {
...
idMatX idMatX::Transpose( void ) const {
  idMatX transpose;
  int i, j;
...
idMatX &idMatX::TransposeSelf( void ) {
  *this = Transpose();
  return *this;
}

idMatX idMatX::Inverse( void ) const {
  idMatX invMat;
...
bool idMatX::InverseSelf( void ) {

  assert( numRows == numColumns );

  switch( numRows ) {
    case 1:
      if ( qkmath::fabs( mat[0] ) < MATRIX_INVERSE_EPSILON ) {
        return false;
      }
      mat[0] = 1.0f / mat[0];
      return true;
...
idMatX idMatX::InverseFast( void ) const {
  idMatX invMat;
...
bool idMatX::InverseFastSelf( void ) {
...
idVecX idMatX::Multiply( const idVecX &vec ) const {
  idVecX dst;
...
idMatX idMatX::Multiply( const idMatX &a ) const {
  idMatX dst;
...
idVecX idMatX::TransposeMultiply( const idVecX &vec ) const {
  idVecX dst;
...
idMatX idMatX::TransposeMultiply( const idMatX &a ) const {
  idMatX dst;
...
void idMatX::Multiply( idVecX &dst, const idVecX &vec ) const {
#ifdef MATX_SIMD
  SIMDProcessor->MatX_MultiplyVecX( dst, *this, vec );
#else
  int i, j;
...
void idMatX::MultiplyAdd( idVecX &dst, const idVecX &vec ) const {
#ifdef MATX_SIMD
...
void idMatX::MultiplySub( idVecX &dst, const idVecX &vec ) const {
#ifdef MATX_SIMD
...
void idMatX::TransposeMultiply( idVecX &dst, const idVecX &vec ) const {
#ifdef MATX_SIMD
  SIMDProcessor->MatX_TransposeMultiplyVecX( dst, *this, vec );
#else
  int i, j;
...
void idMatX::TransposeMultiplyAdd( idVecX &dst, const idVecX &vec ) const {
#ifdef MATX_SIMD
...
void idMatX::TransposeMultiplySub( idVecX &dst, const idVecX &vec ) const {
#ifdef MATX_SIMD
...
void idMatX::Multiply( idMatX &dst, const idMatX &a ) const {
#ifdef MATX_SIMD
  SIMDProcessor->MatX_MultiplyMatX( dst, *this, a );
#else
  int i, j, k, l, n;
...
void idMatX::TransposeMultiply( idMatX &dst, const idMatX &a ) const {
#ifdef MATX_SIMD
  SIMDProcessor->MatX_TransposeMultiplyMatX( dst, *this, a );
#else
  int i, j, k, l, n;
...
const float *idMatX::ToFloatPtr( void ) const {
  return mat;
}

float *idMatX::ToFloatPtr( void ) {
  return mat;
}
