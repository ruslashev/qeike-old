...
idToken::idToken( const idToken *token ) {
  *this = *token;
}
...
void idToken::operator=( const char *text) {
  *static_cast<idStr *>(this) = text;
}

void idToken::operator=( const idStr& text ) {
  *static_cast<idStr *>(this) = text;
}

double idToken::GetDoubleValue( void ) {
  if ( type != TT_NUMBER ) {
    return 0.0;
  }
...
float idToken::GetFloatValue( void ) {
  return (float) GetDoubleValue();
}

unsigned int	idToken::GetUnsignedIntValue( void ) {
  if ( type != TT_NUMBER ) {
    return 0;
  }
  if ( !(subtype & TT_VALUESVALID) ) {
...
int idToken::GetIntValue( void ) {
  return (int) GetUnsignedIntValue();
}

int idToken::WhiteSpaceBeforeToken( void ) const {
  return ( whiteSpaceEnd_p > whiteSpaceStart_p );
}

void idToken::AppendDirty( const char a ) {
