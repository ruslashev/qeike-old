...
int idWinding::Split( const idPlane &plane, const float epsilon, idWinding **front, idWinding **back ) const {
  ...
  dists = (float *) alloca( (numPoints+4) * sizeof( float ) );
  sides = (unsigned char *) alloca( (numPoints+4) * sizeof( unsigned char ) );
  ...
  *back = b = new idWinding(maxpts);
  ...
  if ( f->numPoints > maxpts || b->numPoints > maxpts ) {
    die( "idWinding::Split: points exceeded estimate." );
  }
...
idWinding *idWinding::Clip( const idPlane &plane, const float epsilon, const bool keepOn ) {
  ...
  newPoints = (idVec5 *) alloca( maxpts * sizeof( idVec5 ) );
  newNumPoints = 0;
...
bool idWinding::Check( bool print ) const {
  ...
  if ( numPoints < 3 ) {
    if ( print ) {
      printf( "idWinding::Check: only %i points.", numPoints );
    }
    return false;
  }
  ...
  if ( area < 1.0f ) {
    if ( print ) {
      printf( "idWinding::Check: tiny area: %f", area );
    }
    return false;
  }
  ...
  for ( i = 0; i < numPoints; i++ ) {
    ...
    for ( j = 0; j < 3; j++ ) {
      if ( p1[j] >= MAX_WORLD_COORD || p1[j] <= MIN_WORLD_COORD ) {
        if ( print ) {
          printf( "idWinding::Check: point %d outside world %c-axis: %f", i, 'X'+j, p1[j] );
        }
        return false;
      }
    }
    ...
    if ( d < -ON_EPSILON || d > ON_EPSILON ) {
      if ( print ) {
        printf( "idWinding::Check: point %d off plane.", i );
      }
      return false;
    }
    ...
    if ( dir.Length() < ON_EPSILON) {
      if ( print ) {
        printf( "idWinding::Check: edge %d is degenerate.", i );
      }
      return false;
    }
    ...
    for ( j = 0; j < numPoints; j++ ) {
      ...
      if ( d > edgedist ) {
        if ( print ) {
          printf( "idWinding::Check: non-convex." );
        }
        return false;
      }
    }
...
void idWinding::RemoveEqualPoints( const float epsilon ) {
  ...
  for ( i = 0; i < numPoints; i++ ) {
    if ( (p[i].ToVec3() - p[(i+numPoints-1)%numPoints].ToVec3()).LengthSqr() >= epsilon * epsilon  ) {
      continue;
    }
    numPoints--;
...
void idWinding::RemoveColinearPoints( const idVec3 &normal, const float epsilon ) {
  ...
  for ( i = 0; i < numPoints; i++ ) {
    ...
    if ( qkmath::fabs( edgeNormal * p[(i+1)%numPoints].ToVec3() - dist ) > epsilon ) {
      continue;
    }
...
void idWinding::AddToConvexHull( const idWinding *winding, const idVec3 &normal, const float epsilon ) {
  ...
  newHullPoints = (idVec5 *) alloca( maxPts * sizeof( idVec5 ) );
  hullDirs = (idVec3 *) alloca( maxPts * sizeof( idVec3 ) );
  hullSide = (bool *) alloca( maxPts * sizeof( bool ) );
...
void idWinding::AddToConvexHull( const idVec3 &point, const idVec3 &normal, const float epsilon ) {
  ...
  hullDirs = (idVec3 *) alloca( numPoints * sizeof( idVec3 ) );
  hullSide = (bool *) alloca( numPoints * sizeof( bool ) );
  ...
  hullPoints = (idVec5 *) alloca( (numPoints+1) * sizeof( idVec5 ) );
...
idWinding *idWinding::TryMerge( const idWinding &w, const idVec3 &planenormal, int keep ) const {
  ...
  for ( i = 0; i < f1->numPoints; i++ ) {
    ...
    for ( j = 0; j < f2->numPoints; j++ ) {
      ...
      for (k = 0; k < 3; k++ ) {
        if ( qkmath::fabs((*p1)[k] - (*p4)[k]) > 0.1f ) {
          break;
        }
        if ( qkmath::fabs((*p2)[k] - (*p3)[k]) > 0.1f ) {
          break;
        }
      }
      if ( k == 3 ) {
...
void idWinding::RemovePoint( int point ) {
  if ( point < 0 || point >= numPoints ) {
    die( "idWinding::removePoint: point out of range" );
  }
...
void idWinding::InsertPoint( const idVec3 &point, int spot ) {
  ...
  if ( spot > numPoints ) {
    die( "idWinding::insertPoint: spot > numPoints" );
  }

  if ( spot < 0 ) {
    die( "idWinding::insertPoint: spot < 0" );
  }
...
bool idWinding::InsertPointIfOnEdge( const idVec3 &point, const idPlane &plane, const float epsilon ) {
  ...
  if ( qkmath::fabs( plane.Distance( point ) ) > epsilon ) {
    return false;
  }
  ...
  for ( i = 0; i < numPoints; i++ ) {
    ...
    if ( qkmath::fabs( normal * point - dist ) > epsilon ) {
      continue;
    }
...
bool idWinding::LineIntersection( const idPlane &windingPlane, const idVec3 &start, const idVec3 &end, bool backFaceCull ) const {
  ...
  if ( qkmath::fabs(front - back) < 0.0001f ) {
    mid = end;
  }
  else {
...
idFixedWinding::idFixedWinding( void ) {
  numPoints = 0;
  p = data;
  allocedSize = MAX_POINTS_ON_WINDING;
}

idFixedWinding::idFixedWinding( int n ) {
  numPoints = 0;
  p = data;
  allocedSize = MAX_POINTS_ON_WINDING;
}

idFixedWinding::idFixedWinding( const idVec3 *verts, const int n ) {
  int i;
...
idFixedWinding::idFixedWinding( const idVec3 &normal, const float dist ) {
  numPoints = 0;
  p = data;
  allocedSize = MAX_POINTS_ON_WINDING;
  BaseForPlane( normal, dist );
...
idFixedWinding::idFixedWinding( const idPlane &plane ) {
  numPoints = 0;
  p = data;
  allocedSize = MAX_POINTS_ON_WINDING;
  BaseForPlane( plane );
...
idFixedWinding::idFixedWinding( const idWinding &winding ) {
  int i;
...
idFixedWinding::idFixedWinding( const idFixedWinding &winding ) {
  int i;
...
idFixedWinding &idFixedWinding::operator=( const idWinding &winding ) {
  int i;
...
void idFixedWinding::Clear( void ) {
  numPoints = 0;
}
...
bool idFixedWinding::ReAllocate( int n, bool keep ) {
	...
	if ( n > MAX_POINTS_ON_WINDING ) {
		printf("WARNING: idFixedWinding -> MAX_POINTS_ON_WINDING overflowed\n");
		return false;
	}
	return true;
}
...
int idFixedWinding::Split( idFixedWinding *back, const idPlane &plane, const float epsilon ) {
	...
	unsigned char	sides[MAX_POINTS_ON_WINDING+4];
	float	dot;
	int		i, j;
	idVec5 *p1, *p2;
	idVec5	mid;
